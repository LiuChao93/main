...
We achieve that by working with the `Supplier` model.
Each field in a Supplier is implemented as a separate class (e.g. a `Name` object represents the supplier's name).
That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a supplier.
...
to link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-0c6b6abcfac8c205e075294f25e851fe[`seedu.address.ui.SupplierCard`].
...
```
into link:https://github.com/nus-cs2103-AY1920S1/addressbook-level3/commit/2758455583f0101ed918a318fc75679270843a0d#diff-12580431f55d7880578aa4c16f249e71[`main/resources/view/SupplierListCard.fxml`].
...
We change the constructor of `Supplier` to take a `Remark`.
...
TIP: Use the `Find Usages` feature in IntelliJ IDEA on the `Supplier` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedSupplier` into `json` with the help of an external library -- Jackson.
Let's update `JsonAdaptedSupplier` to work with our new `Supplier`!
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Supplier`.
Since all fields in a `Supplier` are immutable, we create a new instance of a `Supplier` with the values that we want and save it with `Model#setSupplier()`.
